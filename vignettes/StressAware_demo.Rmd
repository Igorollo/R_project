...
model_file_path <- file.path(project_root, "model", "stress_rf.pkl") # Assuming 'model' folder at project root
if (file.exists(model_file_path)) {
  ...
  # Although load_stress_model is typically called within classify_stress_windows,
  # explicitly loading it here might be useful if other parts of the Rmd
  # need it directly.
  tryCatch({
    # Assuming load_stress_model is defined in model.R and sourced by server.R
    # If not, you might need to source model.R here:
    # source(file.path(r_script_path, "model.R"))
    if (exists("load_stress_model", mode = "function")) {
       # This call ensures the model is loaded into Python environment within Rmd
  ...
  warning(paste0("Stress classification model '", model_file_path, "' not found for Rmd. ",
                 "Stress classification features will not be available in report. Please ensure the model file is in the 'model' directory relative to your project root."))
}
...
if (is.null(hr_series_obj) || is.null(classified_stress_data) || nrow(classified_stress_data) == 0) {
  ...
  hr_data_dir <- file.path(project_root, "hr_data") # Assuming 'hr_data' is at the project root
  sample_file_path <- file.path(hr_data_dir, "3.txt") # Use one of your sample files

  # Source required functions for demo if not already available
  # This is crucial for standalone knitting.
  # If running from Shiny, these should already be sourced in server.R's environment.
  tryCatch({
    ...
    source(file.path(r_script_path, "model.R")) # Source model functions again for direct knitting
  ...
  # Load raw data and preprocess for demo purposes if params are NULL
  if (file.exists(sample_file_path) && exists("preprocess_hr") && exists("extract_hrv_features") && exists("create_hrseries")) {
      tryCatch({
        ...
        preproc_results_demo <- preprocess_hr(
          ...
          time_col = "Time",
          rr_col = "rr_ms",
          resample_rate = 4, # Default values for demo
          malik_threshold = 0.2
        )
        ...
        # Classify stress for demo if features are available
...
# Default plot_types if not provided by Shiny (or if demo mode)
...
# Check data availability for conditional content rendering
...
if (data_available && "stress_classification" %in% plot_types && !is.null(classified_stress_data) && nrow(classified_stress_data) > 0) {
  # Assuming plot method of HRSeries object is available and handles "stress_classification" type
  # or you have a separate plotting function for classified_stress_data
  # For example, let's create a simple plot here:
  
  p_stress <- ggplot(classified_stress_data, aes(x = window_end_time, y = stress_probability, color = as.factor(stress_prediction))) +
    geom_line() +
    geom_point(aes(y = as.numeric(stress_prediction) * 0.9 + 0.05)) + # Small offset for clear points
    geom_hline(yintercept = 0.5, linetype = "dashed", color = "grey") +
    scale_color_manual(values = c("0" = "forestgreen", "1" = "firebrick"),
                       labels = c("0" = "Non-Stress", "1" = "Stress")) +
    labs(title = "Stress Probability and Prediction Over Time",
         x = "Time",
